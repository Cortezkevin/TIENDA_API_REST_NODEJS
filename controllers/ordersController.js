 ...
 */
 const getAllItems = async (req, res) => {
    try {    
        const user = req.user;
        const data = await ordersModel.findAllData({});                
        //data.set("categoryId", undefined, {strict: false});
        res.send({data, user});
    } catch (error) {
        console.log("ERROR DATA ORDERS -------", error);
...
const createItem = async (req, res) => {
    try {
        //req = matchedData(req);
        const {body} = req;
        const data = await ordersModel.create(body);
...
const updateItem = async (req, res) => {
    try {
        ...
        const data = await ordersModel.findOneAndUpdate(id, body);
...
const deleteItem = async (req, res) => {
    try {
        ...
        const data = await ordersModel.delete({_id: id});
        const user = req.user;
        res.send({data, user});
    } catch (error) {
